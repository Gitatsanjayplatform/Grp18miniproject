...
function updateSummary() {
  ...
  let car = 0;
  document
    .querySelectorAll('input[name="engineoptionradio"]')
    .forEach((radio) => {
      if (radio.checked) {
        const engineOption = radio.value;
        if (engineOption === "220i M Sport") {
          car = 4350000;
          engineamount = 0;
        } else if (engineOption === "220i M Sport Pro") {
          engineamount = 200000;
          car = 4550000;
        } else if (engineOption === "220d M Sport") {
          engineamount = 290000;
          car = 4640000;
        }
        document.getElementById(
          "car_variant"
        ).textContent = `:   ${engineOption}`;
      }
    });

  document
    .querySelectorAll('input[name="exterioroptionradio"]')
    .forEach((radio) => {
      if (radio.checked) {
        const exteriorOption = radio.value;
        document.getElementById(
          "car_exterior_color"
        ).textContent = `: ${exteriorOption}`;
  ...
  document
    .querySelectorAll('input[name="interioroptionradio"]')
    .forEach((radio) => {
      if (radio.checked) {
        const interiorOption = radio.value;
        document.getElementById(
          "car_interior_color"
        ).textContent = `: ${interiorOption}`;
  ...
  const caramount = 4350000 + chargeamount;

  document.getElementById(
    "engineamount"
  ).textContent = `₹${car.toLocaleString()}`;
  document.getElementById(
    "exterioramount"
  ).textContent = `₹${exterioramount.toLocaleString()}`;
  document.getElementById(
    "interioramount"
  ).textContent = ` ₹${interioramount.toLocaleString()}`;
  document.getElementById(
    "car_additional_charges"
  ).textContent = `₹${chargeamount.toLocaleString()}`;
  document.getElementById(
    "car_Amount"
  ).textContent = `₹${caramount.toLocaleString()}`;
}
...
function scrollToDivengine() {
  ...
  let scrollToY =
    engine_container.getBoundingClientRect().top + window.scrollY - offset;
...
function scrollToDivexterior() {
  ...
  let offset = 100; // Set the desired offset (in pixels) from the top
  let scrollToY =
    Exterior_container.getBoundingClientRect().top + window.scrollY - offset;
...
function scrollToDivinterior() {
  ...
  let offset = 100; // Set the desired offset (in pixels) from the top
  let scrollToY =
    Interior_container.getBoundingClientRect().top + window.scrollY - offset;
...
function updatePrice() {
  ...
  radioButtons1.forEach((radioButton) => {
    if (radioButton.checked) {
      if (radioButton.value === "220i M Sport Pro") {
        engine1.style.display = "none";
        engine2.style.display = "block";
        engine3.style.display = "none";
        enginename = "220i M Sport Pro";
        engineamount = 200000;
      } else if (radioButton.value === "220d M Sport") {
        engine1.style.display = "none";
        engine2.style.display = "none";
        engine3.style.display = "block";
        enginename = "220d M Sport";
        engineamount = 290000;
      } else if (radioButton.value === "220i M Sport") {
        engine1.style.display = "block";
        engine2.style.display = "none";
        engine3.style.display = "none";
        enginename = "220i M Sport";
        engineamount = 0;
      }
    }
  });
